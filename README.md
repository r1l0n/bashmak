...
На ответ ручки бот не смотрит: она поднимает игру и только потом отчитывается,
так что `500` приходит уже поверх запущенной, а иногда ответа нет вовсе. Вслух
поэтому звучит условленная фраза при любом исходе, а код ответа и обрывы
соединения видны в логе (`grep 'ручка запуска' logs/bashmak.log`).
