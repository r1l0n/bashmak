...
только ~3 млрд. По скорости это уровень трёхмиллиардной плотной модели, по
качеству — заметно выше. В памяти при этом лежат все веса целиком, отсюда
разница в размере файла (при 45 ГБ ОЗУ роли не играет).

Отличаются они не скоростью, а характером. У GigaChat русский родной:
обучалась на нём с нуля, и токенизатор режет русский текст вдвое экономнее
западных — при равных tok/s это вдвое больше произнесённого текста. У Qwen3
шире знания и лучше рассуждения, но русский «выученный», и токенов на ту же
фразу уходит больше.

Плотные модели крупнее 14B для голоса не годятся в принципе: 32B на десяти
ядрах даёт 2–3 tok/s, то есть ответ в три предложения синтезируется полминуты.
...
3. Настроить подключение мастером — он спросит параметры, проверит их и
   поднимет сервис:
...
```
   ...
   Проще всего вставить туда ссылку `vless://` из панели целиком: тогда все
   поля разбираются автоматически. Руками их вводить тоже можно.

   Мастер проверяет значения **до** записи. Это не педантизм: опечатка в
   ключе Reality роняет sing-box на старте с `illegal base64 data at input
   byte 12`, следом отваливается туннель, Discord и бот, а выглядит всё как
   проблема с сетью.

   Шаги 2 и 3 можно сделать одной командой — `./scripts/setup.sh --vpn`
   поставит sing-box, если его нет, запустит мастер и сразу пропишет
   маркировку трафика в юните.
...
- «**Башмак**, громкость 70» — выставит именно столько процентов, а не сдвинет
  на шаг, как «погромче». Число можно и словом: «громкость пятьдесят»;
- имя можно ставить где угодно: «включи Кино, **Башмак**», «включи Кино,
  **Башмак**, пожалуйста» — оно вырезается, остальное уходит в работу;
...
(`llm.history_turns`, по умолчанию три) и забывает их после двух минут тишины
(`llm.history_ttl_s`). Глубина маленькая намеренно — на CPU длинный контекст
разбавляет вопрос, и модель начинает отвечать на фон. Отвечает бот всегда одним
предложением: хотите продолжения — спросите ещё раз.

Ник говорящего в вопрос не подмешивается, он уходит в системную часть промпта.
Иначе модель принимает его за тему разговора: человеку с ником GTA на «какая
игра?» она отвечала «Grand Theft Auto».

«Завали ебало, Башмак» (а также «заткнись», «замолчи», «тишина», «стоп») — бот
обрывает фразу на полуслове, выключает музыку и молчит: ответ на такую команду
был бы очередной репликой. Заодно выбрасывается всё, что стояло в очереди, и
контекст разговора.
...
Бюджет ядер по умолчанию рассчитан на 10 физических: `llm.threads: 7`,
`stt.workers: 2` по 2 потока, `tts.threads: 2`, VAD и остальное — на основном
лупе.

Сумма больше десяти, и это осознанно: считать надо не сумму, а пик
одновременной работы. Этапы одной реплики идут по очереди — распознали,
ответили, озвучили, — так что LLM и TTS не пересекаются никогда. Пересекаются
только llama-server и STT, когда бот отвечает одному, а другой уже говорит:
это 7 + 4 = 11 потоков, лёгкая переподписка без последствий.

Наращивать `llm.threads` дальше смысла мало: на CPU инференс упирается в
пропускную способность памяти, а не в вычисления, и отдача от потоков выходит
на плато к шести-восьми. Если бот тормозит при трёх и более говорящих,
снижайте `llm.threads`, а не `stt.workers`: распознавание должно успевать,
иначе речь начнёт теряться в буферах.

`llm.mlock: true` закрепляет веса в памяти. При 45 ГБ ОЗУ это бесплатно и
исключает единственный сценарий, где бот внезапно замолкает на секунды:
часть модели вытеснили под page cache, и её читают с диска посреди ответа.
...
Полосы уровня — по каждому говорящему: слева громкость входа, справа оценка
VAD рядом с порогом. Цвет по мнению VAD, а не по громкости: важно не
«шумно», а слышит ли бот в этом речь. Видно сразу, у кого микрофон тихий и
...
`config.yaml`: конфиг говорит, что бот хотел, а cmdline — что на самом деле
загружено. Разойтись они могут запросто, если сервер не перезапускали после
правки конфига.

Стадия «речь» — это сколько звучит сам ответ. Она обычно и есть самая
длинная часть «всего»: бот ждёт, пока фраза доиграет, прежде чем считать
реплику законченной.
