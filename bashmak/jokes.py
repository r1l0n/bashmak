...
Берутся из RSS-ленты, а не из головы модели: анекдот держится на неожиданной
концовке, и 8B-lite выдаёт вместо неё присказку. Модель остаётся фолбэком — на
случай, когда сети нет или источник лёг.

Лента отдаёт десяток анекдотов за запрос и кешируется: просьба рассказать — это
следующий из уже разобранной пачки, а не поход в сеть. Рассказанные помнятся по
guid, чтобы подряд не повторяться.

Текст не фильтруется: что пришло, то и звучит.
"""
...
#: Сколько рассказанных помнить. Заметно больше пачки: за время жизни кеша
#: лента отдаёт одно и то же, и памяти в десяток записей хватит на один круг.
RECENT_LIMIT = 60
...
class JokeTeller:
    ...
    Живёт на боте, а не на голосовой сессии: пачка и список рассказанного общие
    для всех каналов, иначе в соседнем звучал бы тот же анекдот.
    """
    ...
    async def _from_llm(self) -> str:
        ...
        Без явного слота: оба слота llama-server заняты диалогом и
        классификатором (см. client.CHAT_SLOT), и отдавать один из них под
        разовый запрос — значит выбить чужой кеш префикса.
        """
