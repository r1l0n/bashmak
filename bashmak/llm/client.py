...
#:
#: Диалог и классификатор намерений ходят в одну модель, но с разными
#: системными частями. В общем слоте они вытирают кеш друг другу, и каждая
#: реплика платит полным префиллом персоны — сотни токенов на CPU.
#: Требует --parallel 2 в scripts/run_llama_server.sh.
CHAT_SLOT = 0
INTENT_SLOT = 1
...
def _render_vikhr(messages: list[dict[str, str]]) -> str:
    ...
    Отдельная системная роль важна: она стоит первой и целиком попадает в кеш
    префикса llama-server. У самой YandexGPT-5-Lite такого блока нет, инструкции
    пришлось бы вклеивать в первую реплику пользователя — то есть в ход, который
    первым вытесняется из истории, и персона пересчитывалась бы каждый раз.
...
def _server_error(response: httpx.Response) -> str:
    ...
    raise_for_status() отбрасывает тело, а llama-server пишет в нём, что именно
    случилось: ошибку шаблона чата, переполненный контекст, неизвестный
    параметр.
    """
    detail = ""
    try:
...
class LlmClient:
    def __init__(self, cfg) -> None:  # noqa: ANN001 — bashmak.config.Section
        ...
        # очереди: мимо очереди в LLM ходит ещё и intent-классификатор. Два
        # слота сервера нужны ради двух кешей префикса, а не ради параллельного
        # счёта — на CPU два инференса одной модели делят те же ядра и кеш.
    ...
    async def complete(
        ...
        *,
        ...
        ``slot`` — в каком слоте llama-server считать (см. :data:`CHAT_SLOT`).
        Подстраховка: сервер и сам выбирает слот по длиннейшему общему префиксу,
        а сборка, которая поля не знает, просто его игнорирует.
        """
        ...
        async with self._inference:
            for attempt in range(retries + 1):
                try:
                    ...
                    data = response.json()
                    # Отличает «модель думала 5.8 с» от «досчитала до n_predict,
                    # а хвост выбросил clean_reply»: время одинаковое, лечится
                    # разным.
                    log.debug(
