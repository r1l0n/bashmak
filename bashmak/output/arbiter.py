...
Вариант с паузой музыки, описанный в ТЗ, в discord.py не работает:
``voice_client`` держит ровно один источник, вызвать ``play()`` поверх паузы
нельзя, пришлось бы останавливать трек и пересоздавать ``FFmpegPCMAudio`` с
``-ss <позиция>`` — это рывок, потеря позиции при живом стриме и лишний коннект
к YouTube на каждую реплику.

Поэтому источник один на всё время: :class:`MixerSource`. Он складывает музыку
с речью и приглушает музыку, пока бот говорит. Ничего не перезапускается,
приоритет TTS соблюдён, позиция трека сохраняется.
...
class MixerSource(discord.AudioSource):
    ...
    def __init__(
        ...
        *,
        ...
        # Даккинг относительный: «приглушить до четверти» означает четверть от
        # текущей громкости, а не абсолютные 0.25. При абсолютном значении
        # после пары команд «потише» музыка становилась бы громче, пока бот
        # говорит.
...
class OutputArbiter:
    ...
    def __init__(self, cfg, loop: asyncio.AbstractEventLoop) -> None:  # noqa: ANN001
        ...
        # бросает свою реплику. Без счётчика чистка буфера пропустила бы одно
    ...
    def interrupt(self) -> None:
        ...
        Вызывается не из лупа арбитра, а из обработки команды молчания и из
        закрытия сессии. Ждать здесь нечего: сдвиг счётчика — обычное
        присваивание, буфер чистится под своим локом.
        """
        self._epoch += 1
        self.source.clear_tts()
