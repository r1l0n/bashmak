...
В канал уходит один поток, а претендентов на него два: ответы бота и музыка.
Паузой музыки это не решается: ``voice_client`` держит ровно один источник, и
вызвать ``play()`` поверх паузы нельзя — пришлось бы останавливать трек и
пересоздавать ``FFmpegPCMAudio`` с ``-ss <позиция>``, то есть рывок, потеря
позиции при живом стриме и лишний коннект к YouTube на каждую реплику.

Поэтому источник один на всё время: :class:`MixerSource`. Он складывает музыку
с речью и приглушает музыку, пока бот говорит.

``read()`` вызывается плеером discord.py из отдельного потока каждые 20 мс, так
что всё в нём должно сводиться к арифметике над 960 сэмплами.
"""
...
class MixerSource(discord.AudioSource):
    ...
    def __init__(
        ...
        *,
        ...
        # Даккинг относительный: четверть от текущей громкости, а не абсолютные
        # 0.25. При абсолютном значении после пары команд «потише» музыка
        # становилась бы громче, пока бот говорит.
        ...
        # Реплика приходит кусками, между ними буфер пустеет — без удержания
        # фон на каждой такой паузе всплывает и снова ныряет.
...
class OutputArbiter:
    ...
    def __init__(self, cfg, loop: asyncio.AbstractEventLoop) -> None:  # noqa: ANN001
        ...
        # бросает остаток. Без счётчика чистка буфера снимает только текущее
        # предложение, а следующее досинтезировалось бы и зазвучало.
        self._epoch = 0
    ...
    async def speak(self, chunks: AsyncIterator[tuple[bytes, int]]) -> None:
        ...
        # Поколение снимается до лока: реплика, которую оборвали, пока она
        # стояла в очереди на выход, звучать не должна.
        epoch = self._epoch
        ...
        async with contextlib.aclosing(chunks) as stream:
            ...
            async with self._lock:
                ...
                # Ждать событие без таймаута нельзя: его выставляет поток
                # плеера, а он крутится только пока живо голосовое соединение.
                # Оборвалось — и вся очередь LLM встанет намертво до рестарта.
                timeout = pushed / BYTES_PER_SECOND + SPEAK_TIMEOUT_SLACK
                try:
                    # Отдельной стадией: это самая долгая часть реплики, без неё
                    # разбивка не сходится с «всего».
    ...
    def interrupt(self) -> None:
        """Прервать вывод: и то, что звучит, и то, что ещё синтезируется."""
        self._epoch += 1
        self.source.clear_tts()
