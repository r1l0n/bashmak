...
Отдельная wake-word модель не нужна: STT расшифровывает всю речь, которую
выделил VAD, поэтому достаточно нечёткого поиска по тексту. Whisper регулярно
слышит «башмака», «бошмак», «башмачок», точное сравнение здесь не работает, а
rapidfuzz ловит склонения и типичные ошибки распознавания.

Сравнение пословное, а не по всей строке: ``fuzz.ratio`` по длинной фразе не
срабатывает, а ``partial_ratio`` даёт ложные совпадения на любом похожем куске.

Нормализация используется только для сравнения. Наружу уходит исходный текст с
вырезанным именем: LLM и поиск музыки получают «Расскажи анекдот про кота!», а
не «расскажи анекдот про кота».

Имя может стоять в любом месте фразы: «Башмак, включи дору», «включи дору,
Башмак», «включи дору, Башмак, пожалуйста». Поэтому вырезаются все его
вхождения, а остаток склеивается обратно.
"""
...
class WakeWordFilter:
    ...
    def match(self, text: str) -> WakeMatch | None:
        ...
        for found in _WORD.finditer(text):
            ...
            # Длина отсекает мусор до вызова fuzz: «а», «ну», «да».
            if len(word) < 4:
                continue
        ...
        # Наружу отдаётся лучшее совпадение: по нему видно, как имя прозвучало
        # на самом деле.
    ...
    def _strip_name(text: str, hits: list[tuple[re.Match[str], float]]) -> str:
        ...
        Имя — обращение, а не начало команды: оно встречается и в конце
        («включи дору, Башмак»), и в середине («включи дору, Башмак,
        пожалуйста»), и дважды за фразу. Берётся всё, кроме самого имени;
        вариант «всё после имени» терял бы половину запроса.
        """
        pieces: list[str] = []
        cursor = 0
        for found, _ in hits:
