...
выделил VAD, поэтому достаточно нечёткого поиска по тексту — распознавание
регулярно слышит «башмака», «бошмак», «башмачок».
...
именем: LLM и поиск музыки получают «Расскажи анекдот про кота!», а не
«расскажи анекдот про кота».

Имя может стоять где угодно во фразе («включи дору, Башмак, пожалуйста»),
поэтому вырезаются все его вхождения, а остаток склеивается обратно.
"""
...
class WakeWordFilter:
    ...
    def _strip_name(text: str, hits: list[tuple[re.Match[str], float]]) -> str:
        ...
        Берётся всё, кроме самого имени: вариант «всё после имени» терял бы
        половину запроса, когда обращение стоит в конце.
        """
        pieces: list[str] = []
        cursor = 0
        for found, _ in hits:
