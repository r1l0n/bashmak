...
class Intent(str, Enum):
    ...
    JOKE = "joke"
    #: «Дай новый сценарий» — кодовая фраза запуска игры (bashmak/game.py).
    GAME = "game"
...
#
# В SILENCE нет «тише»/«потише» — это громкость («сделай потише») — и нет голого
# «хватит»: оно занято правилом ниже («хватит музыки»).
_RULES: tuple[tuple[Intent, re.Pattern[str]], ...] = (
    ...
    # Выше музыкальных правил: «включи анекдот» — это анекдот, а не поиск на
    ...
    # «Пошути» только в повелительном: \b не даёт зацепить «он хотел пошутить».
    (Intent.JOKE, re.compile(r"\bанекдот\w*\b|\bпошути\b")),
    # Кодовая фраза запуска игры: «Башмак, дай новый сценарий». Тоже выше
    # музыкальных правил — иначе «запусти новый сценарий» ушло бы искать
    # «новый сценарий» на YouTube.
    #
    # Ловится широко: распознавание теряет то глагол, то прилагательное. Но
    # обязательно по существительному вместе со спутником — голое «сценарий»
    # встречается и в обычном разговоре. Между ними допускается пара слов:
    # «дай мне ещё один сценарий».
    #
    # «ё» к этому моменту заменена на «е» (см. classify_by_rules), поэтому в
    # списке «еще», а не «ещё».
    (
        Intent.GAME,
        re.compile(
            r"\b(?:нов\w*|очередн\w*|еще|друг\w*|дай\w*|давай\w*|подкинь\w*|скинь\w*"
            r"|запусти\w*|врубн?и\w*|включ\w*|хочу|нужен|нужны)\W+(?:\w+\W+){0,2}сценари\w*"
        ),
    ),
    ...
    # Выше «прибавь громкость»: с числом это установка, а не шаг.
...
#: «Что-нибудь на свой вкус» — такой же повод считать реплику командой плееру,
#: как и слово «музыка»: названия в ней нет, но включить просят определённо.
#:
#: «ё» к этому моменту заменена на «е» (см. classify_by_rules), поэтому в
...
#:
#: «Что-нибудь», «любой» и «на свой вкус» вычищаются тоже: без этого «включи
#: любой трек» ушло бы искать на YouTube слово «любой». Пустой остаток — это и
#: есть команда «выбери сам».
#:
#: Многословные варианты стоят выше однословных: альтернатива выбирается первой
#: подошедшей, и «на» из «на фон» съело бы начало «на свой вкус».
_FILLER = re.compile(
