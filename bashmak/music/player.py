...
class _FFmpegLog:
    ...
    Объект нарочно без ``fileno()`` — по нему discord.py решает, отдать stderr
    процессу-родителю или качать его себе (см. FFmpegAudio). Родителю нельзя:
    это journal, а не лог бота, и «403 Forbidden» на потоке YouTube остался бы
    неотличим от обычного конца трека.
    """
...
#: Больше не нужно: источников в конфиге единицы, и длинная память выест из них
#: все годные варианты.
RECENT_LIMIT = 30
...
class MusicPlayer:
    def __init__(self, cfg, arbiter: OutputArbiter) -> None:  # noqa: ANN001
        ...
        #: Само по себе, в тишине, оно не заводится.
    ...
    async def _radio_next(self) -> None:
        ...
        что играет, видно в /status и в логе.

        Под guard: таск никто не ждёт, и без него ошибка всплыла бы
        «Task exception was never retrieved» при сборке мусора.
        """
        try:
            track = await search_random(
                self._sources, self._pool, self._search_timeout, self._recent, self._min_views
