...
class _Pretty(logging.Formatter):
    """В штатном режиме — только время и текст.

    Имя модуля и cid добавляют тридцать символов перед каждой строкой. Они
    нужны при ошибках и на уровне DEBUG, там и показываются.
    """
...
def publish_levels(path: Path, payload: dict[str, Any]) -> None:
    ...
    Через временный файл и os.replace: монитор читает этот файл постоянно и не
    должен получить его наполовину записанным. Ошибки подавляются: индикатор не
    повод ронять бота.
    """
...
def setup_logging(cfg: Any = None) -> None:
    ...
    # На DEBUG эти логгеры заглушают полезные строки объёмом.
    ...
    # Эти двое шумят уже на INFO, то есть даже когда discord поднимают до INFO
    # ради отладки голоса. reader пишет строку на каждый RTCP-отчёт (раз в
    # секунду на канал: он считает «неожиданным» всё, кроме ReceiverReport, а
    # Discord штатно шлёт SenderReport), gateway — на каждый кадр голосового WS,
    # потому что discord.py 2.7 добавила туда поле seq, о котором расширение не
    # знает. Обе строки безвредны и обе делают лог нечитаемым.
