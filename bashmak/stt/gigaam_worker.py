...
Пул потоков, а не процессов: onnxruntime отпускает GIL на время ``Run``. Отсюда
одна копия весов в памяти, загрузка модели на старте бота и сегмент, уходящий в
неё без пикла через границу процесса.

Энкодер Conformer считает фактическую длину сегмента, а не окно, дополненное до
30 секунд, — в голосовом чате почти все реплики короткие, и выигрыш здесь, а не
в размере модели.

``threads`` — общий пул onnxruntime на все одновременные распознавания разом,
то есть жёсткий потолок нагрузки, а не множитель по говорящим.
"""
...
class GigaamPool:
    ...
    def __init__(self, cfg) -> None:  # noqa: ANN001 — bashmak.config.Section
        ...
        # время, пока бот сидит в канале. Выигрыш от спина 0.01 с на реплику, а
        # платит за него llama-server своими семью потоками на соседних ядрах.
        ...
        # with_timestamps даёт по токенам logprob — единственный сигнал
        # уверенности, который отдаёт onnx-asr, и на нём держится отсев шума
        # (см. min_avg_logprob). Обычный recognize возвращает голую строку.
        ...
        # эти секунды должны уйти в старт бота, а не в первую фразу.
