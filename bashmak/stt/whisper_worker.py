...
По умолчанию распознаёт GigaAM (``stt.engine: gigaam``). Этот модуль остаётся
рабочим, чтобы вернуться на whisper правкой одной строки конфига, если GigaAM
не справится с конкретным звуком.

faster-whisper держит GIL почти всё время инференса, поэтому пул процессов, а
не потоков, и в каждом процессе своя копия модели (``small`` int8, ~250 МБ).

Контекст создаётся через ``spawn``: fork процесса, в котором уже работает
asyncio-луп и открыты сокеты Discord, приводит к трудноуловимым зависаниям.
"""
