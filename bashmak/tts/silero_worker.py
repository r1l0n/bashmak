...
Ставит ударения, произносит «ё», снимает омографы (за́мок / замо́к) и держит
вопросительную интонацию. Цена — torch в зависимостях и копия модели в каждом
процессе-воркере, поэтому по умолчанию ``workers: 1``.
...
предложение начинает звучать, пока синтезируется второе.
...
def _drop_unsupported(params: dict) -> None:
    ...
    несовпадение конфига и версии модели выглядит как молчащий бот.
    """
...
def _init_worker(model_path: str, params: dict) -> None:
    ...
    _PARAMS = params
    # Первый вызов torch тратит секунды на прогрев (аллокаторы, ядра).
    _synthesize("раз")
...
def _synthesize(text: str) -> tuple[bytes, int]:
    ...
    # Словарём: _drop_unsupported уже вычистил отсюда всё, чего эта версия
    # модели не понимает.
