...
потока приёма: при десятке участников это доли процента CPU и ни одного
примитива синхронизации между потоком роутера пакетов и лупом.

Задержка опроса (30 мс) на порядок меньше порога тишины и на момент нарезки
фраз не влияет.
"""
...
#: одно окно (32 мс речи) на тик (30 мс). Но буфер держит до 30 секунд, и после
#: заминки (GC, спавн воркера STT) один вызов прогнал бы под тысячу инференсов
#: подряд и заблокировал луп на сотни миллисекунд — то есть пропущенные
#: heartbeat'ы Discord и разрыв соединения.
MAX_VAD_WINDOWS_PER_TICK = 16
#: Как часто писать сводку «сколько звука пришло и что о нём думает VAD».
LEVEL_REPORT_INTERVAL = 5.0
#: Затухание полосы индикатора за тик опроса (30 мс). Без него индикатор
#: дёргается вместе с формой волны и читается как рябь; 0.88 — падение примерно
#: за треть секунды.
METER_DECAY = 0.88
...
class VoiceListener:
    ...
    def __init__(
        ...
        #: Для индикатора в мониторе: в отличие от сводки в лог, это «сейчас», а
        #: не накопление за пять секунд.
    ...
    def levels(self) -> dict[int, dict[str, float]]:
        ...
        Чистое чтение накопленного: ввода-вывода в аудиотракте быть не должно,
        файл пишет вызывающий (bashmak/bot.py).
        """
        return {
            user_id: {
    ...
    def _report_level(self, user_id: int, samples: np.ndarray, segmenter: Segmenter) -> None:
        ...
        Отличает «никто не говорил» от «в кадрах шум вместо речи» — в логе оба
        случая одинаково пусты. Пик около нуля — приходит тишина; пик заметный
        при оценке VAD в нуле — приходит не речь.
        """
        now = time.monotonic()
    ...
    async def _handle(self, segment: Segment, cid: str) -> None:
        current_cid.set(cid)
        # Отчёт открывается здесь, а не на первой стадии: иначе из «всего»
        # выпадает пауза, по которой фраза закрылась, — то самое время, которое
        # человек слышит как молчание бота.
