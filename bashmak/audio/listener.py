...
Один asyncio-таск опрашивает буферы всех говорящих. Выбран опрос, а не
будильник из потока приёма: при десятке участников это доли процента CPU и ни
одного примитива синхронизации между потоком роутера пакетов и лупом — то есть
нет и класса багов «залип на локе внутри voice receive».

Задержка опроса (30 мс) на порядок меньше порога тишины (700 мс) и на момент
нарезки фраз не влияет.
"""
...
#: до 30 секунд, и после заминки (GC, спавн воркера STT) один вызов прогнал бы
#: под тысячу инференсов подряд и заблокировал луп на сотни миллисекунд, что
#: означает пропущенные heartbeat'ы Discord и разрыв соединения. С лимитом
#: отставание разгребается за несколько тиков, по 16 окон за раз.
...
#:
#: Без него индикатор дёргается вместе с формой волны и читается как рябь.
#: 0.88 за 30 мс — полоса падает примерно за треть секунды: видно, кто только
#: что говорил, и при этом за реальными паузами она успевает.
METER_DECAY = 0.88
...
class VoiceListener:
    ...
    def _report_level(self, user_id: int, samples: np.ndarray, segmenter: Segmenter) -> None:
        ...
        Позволяет отличить «никто не говорил» от «в кадрах шум вместо речи»:
        аудио доходит до буферов в обоих случаях, фраз не закрывается тоже в
        обоих, и в логе одинаково пусто. Пик около нуля — приходит тишина; пик
        заметный при оценке VAD в нуле — приходит не речь.
        """
        now = time.monotonic()
