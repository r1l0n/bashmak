...
discord-ext-voice-recv отдаёт голос раздельно по SSRC, смешивать ничего не
нужно: кадры раскладываются по буферам.

``write()`` вызывается из потока роутера пакетов, общего на весь голосовой
клиент. Здесь только микс стерео в моно, ресемпл 48→16 кГц и append в дек;
VAD, STT и остальное работают в asyncio-лупе (см. listener.py).

``wants_opus() → False`` означает, что библиотека сама раскодирует Opus и
положит в ``data.pcm`` 48 кГц / 2 канала / s16le.

E2EE библиотека не снимает. Расшифровку DAVE ставит
:mod:`bashmak.audio.voice_recv_patch`; без неё в ``pcm`` приходит шум.
"""
...
class BashmakSink(AudioSink):
    ...
    def _user_id(self, user, data) -> int | None:  # noqa: ANN001 — типы заданы библиотекой
        ...
        ``user`` может быть ``None``: библиотека ищет человека в кэше гильдии, а
        до первого события speaking там пусто. Тогда спрашиваем голосовой клиент
        напрямую — мапу SSRC→id он ведёт сам.
        """
        if user is not None:
            return user.id
    ...
    def write(self, user, data) -> None:  # noqa: ANN001 — сигнатура задана библиотекой
        ...
        try:
            ...
            # участников. Поэтому в лог один раз на пользователя и дальше.
