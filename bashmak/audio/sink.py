...
клиент. Здесь выполняются только микс стерео в моно, ресемпл 48→16 кГц и
append в дек. VAD, STT и остальное работают в asyncio-лупе (см. listener.py).

``wants_opus() → False`` означает, что библиотека сама раскодирует Opus и
положит в ``data.pcm`` 48 кГц / 2 канала / s16le — формат, который ожидает
``discord_pcm_to_mono()``.

E2EE библиотека не снимает. Расшифровку DAVE ставит
:mod:`bashmak.audio.voice_recv_patch` до подключения к каналу; без неё в
``pcm`` приходит шум.
"""
...
class BashmakSink(AudioSink):
    ...
    def write(self, user, data) -> None:  # noqa: ANN001 — сигнатура задана библиотекой
        ...
        try:
            ...
            # участников. Поэтому пишем в лог один раз на пользователя и
            # продолжаем.
