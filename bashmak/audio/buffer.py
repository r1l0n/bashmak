...
Здесь единственное место, где встречаются два мира: поток роутера пакетов
discord-ext-voice-recv (пишет) и asyncio-луп бота (читает). Всё, что делает
писатель, — кладёт готовый numpy-массив под коротким локом. Никакой обработки
в потоке приёма быть не должно: он общий на весь голосовой клиент, и задержка
в нём тормозит приём речи ВСЕХ участников сразу.
"""
