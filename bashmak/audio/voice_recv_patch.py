...
а готовая сессия лежит в ``vc._connection.dave_session``. Здесь остаётся
только вызвать у неё ``decrypt()`` на приёме.
...
def dave_unwrap(session, protocol_version: int, user_id: int | None, media_type, payload: bytes) -> bytes:  # noqa: ANN001
    ...
    Вынесено из ``_process_packet`` отдельной функцией: здесь четыре условия,
    при каждом из которых расшифровывать нельзя, и ошибка в любом даёт либо
    исключение из davey на каждом кадре, либо немой канал, который снаружи
    выглядит рабочим:
...
def take_lowest(buffer) -> object | None:  # noqa: ANN001 — HeapJitterBuffer
    ...
    Замена штатному «выбросить буфер целиком». Обращается к внутренним полям
    ``HeapJitterBuffer``, потому что публичного способа достать один пакет в
...
def _patch_dave_decrypt() -> None:
    ...
    def report_mode(ssrc: int, session, version: int, user_id: int | None) -> None:  # noqa: ANN001
        ...
        Пишется только при смене режима, поэтому строк единицы, а не 50 в
        секунду. Без этого по логу не отличить рабочий приём от искажённого
        звука: в обоих случаях PCM доходит до буферов, но во втором в Opus
        уходит нерасшифрованный шифротекст.
        """
        if user_id is None:
    ...
    def dave_decrypt(self, packet) -> None:  # noqa: ANN001 — rtp.AudioPacket
        ...
        try:
            ...
            # Кадр остаётся как есть: он мог оказаться открытым и ещё
...
def _patch_decoder_resilience() -> None:
    ...
    Перехватывается всё, а не только ``OpusError``: цена ошибки — один кадр
    (20 мс), цена мёртвого потока — вся сессия. Чтобы заплатка не скрывала
    проблемы, первое исключение каждого рода пишется целиком, дальше — каждое
    500-е.
    """
    from discord.ext.voice_recv import opus as recv_opus
