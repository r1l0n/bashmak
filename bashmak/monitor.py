...
Работает отдельным процессом: монитор нужен как раз тогда, когда с ботом
что-то не так, поэтому он не делит с ним event loop и не падает вместе с ним.
Оба источника данных пассивные:
...
IPC нет: бот не знает про монитор, монитор не мешает боту. Запускать и
останавливать его можно в любой момент, в том числе по ssh.
"""
