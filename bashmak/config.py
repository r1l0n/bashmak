...
которого в шаблоне нет, виден как опечатка — раньше он просто не читался, и
человек искал, почему правка не действует.
"""
...
def merge_defaults(
    ...
    Ключ, которого нет в шаблоне, — почти всегда опечатка или переименованная
    настройка. Пишем о нём в лог, но не падаем: лишняя строка в конфиге не повод
    не поднимать бота, а строка на старте всё равно попадёт в journald.
    """
