#!/usr/bin/env bash
#
# Заворачивает в туннель трафик одного сервиса — bashmak.service.
#
# В некоторых сетях Discord заблокирован по IP (SYN дропается, до TLS дело не
# доходит), и обход на уровне DPI не работает. Нужен туннель, но заворачивать в
# него весь сервер не требуется: llama-server слушает 127.0.0.1, Whisper и
# Silero сети не касаются. Наружу ходит только сам бот.
#
# Отбор по cgroup systemd-юнита, а не по UID: не нужно заводить отдельного
# пользователя и переразбивать права на каталог проекта, venv и модели.
# Правило действует ровно на один сервис, ssh и apt остаются на прямом
# маршруте.
#
...
up() {
    ...
    # для этого линка, с доменом «~.», то есть перехватывает все запросы хоста,
    # а не только бота. Здесь нужно обратное: туннель предназначен одному
    # сервису, а хост должен резолвить через свой обычный апстрим. Без этой
    # строки DNS ломается на всей машине, включая apt и git.
    ...
    # поэтому послабление приходится делать и на all.
    ...
    # DNS не исключается: у сервиса свой resolv.conf с 1.1.1.1 (см.
    # deploy/resolv.conf и BindReadOnlyPaths в юните), и эти запросы должны
    # идти тем же маршрутом, что и остальные. Исключение сломало бы схему:
    # имена резолвились бы мимо туннеля или не резолвились вовсе.
...
status() {
    ...
    # «DNS Domain: ~.» означает, что sing-box снова перехватил резолвинг всего
    # хоста; в этом случае DNS перестаёт работать на всей машине.
