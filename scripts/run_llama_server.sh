...
# Шаблон чата не задаём сознательно: бот ходит в /completion со своим готовым
# промптом (bashmak/llm/client.py), мимо чат-слоя llama.cpp — тот на формате
# GigaChat ломается что со штатным шаблоном, что со своим.
exec "$BIN" \
    --model "$MODEL" \
    --host "$HOST" \
    --port "$PORT" \
