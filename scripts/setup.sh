...
case "$PROFILE" in
    ...
    qwen)
        ...
        # Шире общие знания и логика, но русский «выученный»: токенизатор
        # кодирует его расточительнее, и прирост в словах за секунду меньше,
        # чем следует из tok/s. Файл вдвое больше, при 40 ГБ ОЗУ это допустимо.
...
install_apt() {
    ...
    # ffmpeg и libopus обязательны: без ffmpeg не работает музыка, без opus бот
    # не может ни принять, ни отдать голос.
...
setup_venv() {
    ...
    # yt-dlp — отдельно от всех и на каждый запуск.
    #
    # В requirements.txt он нарочно без верхнего пина: YouTube ломает старые
    # версии за недели. Но сверка хеша выше означает, что pip вообще не
    # запускается, пока не изменится сам requirements.txt, — и «без пина» на
    # живом сервере превращается в «заморожен навсегда».
    #
    # Симптом у протухшего yt-dlp обманчивый: метаданные он ещё отдаёт, поэтому
    # бот исправно называет трек вслух, а ffmpeg получает на ссылку потока
    # 403 и не играет ни секунды. Выглядит как поломка звука, а не обновлений.
    if "$PY" -m pip install --quiet --upgrade yt-dlp; then
        ok "yt-dlp обновлён: $("$PY" -c 'import yt_dlp; print(yt_dlp.version.__version__)')"
    else
        warn "yt-dlp обновить не удалось — если музыка молчит, начинать чинить отсюда"
    fi
}
...
# предпочтительнее; если доступны только шарды, качаются все — llama.cpp
# подхватит остальные части по первой.
...
try_fetch_llm_from() {
    ...
    # публикаторов различаются регистром и суффиксами (q4_K_M / Q4_K_M /
    # -Q4_K_M-00001-of-00002).
...
setup_vpn() {
    ...
    # Мастер спрашивает параметры, проверяет их и перезапускает сервис.
    # Отдельным скриптом из-за интерактивного ввода и разбора JSON: на bash
    # это было бы длиннее и хуже проверялось.
